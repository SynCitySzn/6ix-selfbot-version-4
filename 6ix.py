...
def startprint():
    ...
    print(f'''{Fore.RESET}
                                    ...
                                    {Fore.RESET}Developer: - {Fore.RED} [Pix#1337]  
                                    {Fore.RESET}Python version- {Fore.RED} [3.8.0]  
                                    ═══════════════════════════════════════════
...
def BotTokens():
    with open('Data/Tokens/bot-tokeclient.txt', 'a+') as f:
        tokens = {token.strip() for token in f if token}
    for token in tokens:
        yield token

def UserTokens():
    with open('Data/Tokens/user-tokeclient.txt', 'a+') as f:
        tokens = {token.strip() for token in f if token}
    for token in tokens:
        yield token
...
@SIX.command()
async def delLogger(message):
    await message.message.delete()
    global dm_log
    if dm_log == 0:
        dm_log += 1
        await message.send(embed=discord.Embed(title="Delete-Sniper `ON`", color= discord.Color(random.randint(0x000000, 0xFFFFFF))), delete_after=5)
        
    elif dm_log == 1:
        dm_log -= 1
        await message.send(embed=discord.Embed(title="Delete-Sniper `OFF`", color= discord.Color(random.randint(0x000000, 0xFFFFFF))), delete_after=5)
        ...
        try:
            if message.guild is None:
                if message.author == SIX.user:
                    return
                embed=discord.Embed(color= discord.Color(random.randint(0x000000, 0xFFFFFF)),timestamp=message.created_at) 
                embed.set_author(name=f'{message.author}', url=message.author.avatar_url)
                embed.add_field(name="{}".format(message.content), value="l", inline=True)
...
@SIX.event
async def snipe(message,ctx):
    await SIX.process_commands(message)
    try:
        if message.guild is None:
            if message.author == SIX.user:
                return
            embed=discord.Embed(color= discord.Color(random.randint(0x000000, 0xFFFFFF)),timestamp=ctx.message.created_at) 
            embed.set_author(name=message.user, url=message.author.avatar_url)
            embed.add_field(name="{}".format(message.content), value="logged", inline=True)
            await message.channel.send(embed=embed)
    except:
        pass  
...
async def afk(message):
    ...
    if afk_log == 0:
        afk_log += 1
        await message.send(embed=discord.Embed(title="AFK `ON`", color= discord.Color(random.randint(0x000000, 0xFFFFFF))), delete_after=5)
        
    elif afk_log == 1:
        afk_log -= 1
        await message.send(embed=discord.Embed(title="AFK `OFF`", color= discord.Color(random.randint(0x000000, 0xFFFFFF))), delete_after=5)   
@SIX.event                
async def on_message(message):
    ...
    if afk_log == 1:
        ...
        if message.guild is None:
            if message.author == SIX.user:
                return
            embed = discord.Embed(title="Auto Responder", description=f"**Heyy {message.author}**, **{afk_message}**", color= discord.Color(random.randint(0x000000, 0xFFFFFF)))
            await message.channel.send(embed=embed)
    await SIX.process_commands(message)
...
async def help(ctx):
    await ctx.message.delete()
    em = discord.Embed(title="Help menu", description="", color=0x000000)
    ...
    em.set_image(url="https://cdn.discordapp.com/attachments/722157826629304431/742874507051663430/image0.gif")
    em.set_footer(text=f"Logged in as : {ctx.author}", icon_url=ctx.author.avatar_url)
    await ctx.send(embed=em)
@SIX.command()
async def cmd(ctx):
    await ctx.message.delete()
    em = discord.Embed(title="command menu", description="", color=0x000000)
...
async def gifs(ctx):
    await ctx.message.delete()
    em = discord.Embed(title="18+", description="", color=0x000000)
...
async def amc(ctx):
    await ctx.message.delete()
    em = discord.Embed(title="for your account", description="", color=0x000000)
...
async def fun(ctx):
    await ctx.message.delete()
    em = discord.Embed(title="fun commands", description="", color=0x000000)
    ...
    em.add_field(name="`token`", value="Grabs the niggas token.\n", inline=False)
    em.add_field(name="`PLookup`", value="Gives info based off phone number.\n", inline=False)  
    em.add_field(name="`cashapp`", value="Paypal.", inline=False)  
    em.add_field(name="`cashapp`", value="cashapp.", inline=False)  
    em.add_field(name="`Cookie`", value="Cookie grabs people on roblox.\n", inline=False)
    em.add_field(name="`Scam`", value="Grabs the niggas payment methods.\n", inline=False)
    em.add_field(name="`Ddos`", value="Loads up a ip booter.\n", inline=False)
    em.add_field(name="`Dox`", value="Doxx anything brah.\n", inline=False)
    em.add_field(name="`Boot`", value="Boot that nigga offline", inline=False)
...
async def util(ctx):
    await ctx.message.delete()
    em = discord.Embed(title="for your server", description="", color=0x000000)
...
async def hacking(ctx):
    await ctx.message.delete()
    em = discord.Embed(title="hacking commands", description="", color=0x000000)
    ...
    em.add_field(name="`tokenban`",value="bans the token",inline=False)
...
async def wizzing(ctx):
    await ctx.message.delete()
    em = discord.Embed(title="raiding commands", description="", color=0x000000)
...
async def cashapp(beaters, amount: int):
        embed = discord.Embed(title=f"CashApp :heart:", color=0x000000)
...
async def tokenban(ctx, _token):
    ...
    if r.status_code == 400:
       await ctx.send(f"`Account disabled. pussy ass nigga`")
       print(f'[{Fore.RED}+{Fore.RESET}] Account disabled successfully')
    else:
       await ctx.send(f"`Invalid token nigga`")
       
def RED(): 
    RED = discord.Color(random.randint(0xFF00CB, 0xFF00CB))
    return RED
...
async def cfres(beaters,domain):
        ...
        embed.set_footer(text="6ix Selfbot <3", icon_url="https://cdn.discordapp.com/attachments/741052681669443715/745310521796067328/651954313668722693.gif")
        await beaters.send(embed=embed)
        await beaters.message.delete()

@SIX.command(pass_context=True)
async def vpn(beaters):
        embed = discord.Embed(title="SixSec VPN Plans", description="```SixSec Is A VPN Company That Cares Fully About Your Privacy We Are Here To Stop DDoS Attack With Our Anti-DDoS Protection And Our Custom Firewalls, We Provide A No Log Policy We Provide Locations That Fit You Best For Gaming Or Web Surfing, We Are Constantly Updating Our Firewalls To Block New DDoS Attack Methods.```", color=0x000000)
        ...
        embed.set_footer(text="Copyright © 16₆ᵢₓ#1337 ", icon_url="https://cdn.discordapp.com/attachments/722157826629304431/753771704815190147/image0.gif")
        await beaters.send(embed=embed)
        await beaters.message.delete()
...
async def about(beaters):
    embed = discord.Embed(title="About 6ix Selfbot :heart:", description="Developer: Pix#1337\nWebsite: https://github.com/elarii/6ix-selfbot-version-4 Python\nVersion: 3.8.0", color=0x000000)
    embed.set_footer(text="6ix Selfbot <3", icon_url="https://cdn.discordapp.com/attachments/748298591243599902/749417339963047966/image0.gif")
    await beaters.send(embed=embed)
    await beaters.message.delete() 
...
async def Ddos(ctx):
    ...
    print('SENDING BOTNETS TO 125.62.3624362...')
    print('SENDING BOTNETS TO 125.62.3624362...')
    print('[OFFLINE] IP WAS SLAIN BY 6ix<3 BOOTER')
    print('[OFFLINE] IP WAS SLAIN BY 6ix<3 BOOTER')
    print('[OFFLINE] IP WAS SLAIN BY 6ix<3 BOOTER')
    print('[OFFLINE] IP WAS SLAIN BY 6ix<3 BOOTER')
    print('[OFFLINE] IP WAS SLAIN BY 6ix<3 BOOTER')
...
async def cuddle(ctx, user: discord.Member):
    ...
    em.set_footer(text="6ix Selfbot <3", icon_url="https://cdn.discordapp.com/attachments/748298591243599902/749417339963047966/image0.gif")
    await ctx.send(embed=em)
@SIX.command()
...
async def sex(ctx, user: discord.Member):
    ...
    r = requests.get("https://nekos.life/api/v2/img/Random_hentai_gif")
    res = r.json()
    em = discord.Embed(description=SIX.user.name+' *having sex with* '+user.mention, color= discord.Color(random.randint(0x000000, 0xFFFFFF)))
    em.set_image(url=res['url'])
    em.set_footer(text="6ix Selfbot <3", icon_url="https://cdn.discordapp.com/attachments/748298591243599902/749417339963047966/image0.gif")
    await ctx.send(embed=em)    
...
@SIX.command(pass_context=True)
async def kill(beaters, member: discord.Member = None):
...
async def covid(ctx, area: str = "Global"):
    ...
    embed = discord.Embed(title=f"Covid 19 Stats ({area.title()})",colour = discord.Colour(random.randint(0x000000, 0xFFFFFF)),timestamp=ctx.message.created_at)
    embed.add_field(name="New Cases", value=f"{formatted_json['NewConfirmed']:,}",inline=False)
    embed.add_field(name="Total Cases", value=f"{formatted_json['TotalConfirmed']:,}",inline=False)
    embed.add_field(name="New Deaths", value=f"{formatted_json['NewDeaths']:,}",inline=False)
    embed.add_field(name="Total Deaths", value=f"{formatted_json['TotalDeaths']:,}",inline=False)
    embed.add_field(name="New Recovered", value=f"{formatted_json['NewRecovered']:,}",inline=False)
    embed.add_field(name="Total Recovered", value=f"{formatted_json['TotalRecovered']:,}",inline=False)
    embed.set_footer(text=f"6ix selfbot", icon_url=ctx.author.avatar_url)
    await ctx.send(embed=embed)
...
async def urban(ctx, *, args):
		...
		num = 0
		embed = discord.Embed(timestamp=ctx.message.created_at,colour = discord.Colour(random.randint(0x000000, 0xFFFFFF)))
		embed.set_footer(text=f"6ix selfbot", icon_url=ctx.author.avatar_url)
		async with ctx.typing():
