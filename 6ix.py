...
async def dlogger(message):
    ...
    if dm_log == 0:
        dm_log += 1
        await message.send("Delete-Sniper `ON`", delete_after=5)
        
    elif dm_log == 1:
        dm_log -= 1
        await message.send("Delete-Sniper `OFF`", delete_after=5) 
...
async def cuddle(ctx, user: discord.Member):
    ...
    em.set_footerem.set_footer(text=f"Logged in as : {ctx.author}", icon_url=ctx.author.avatar_url)
    await ctx.send(embed=em)
@SIX.command()
